...
class App:
    def __init__(self, root):
        ...
        self.button = tk.Button(self.root, text="Record Coordinates",\
                                command=self.Record_Coord)
        self.button.pack()
    ...
    def Record_Coord(self):
        label = self.entry.get() if self.entry.get() else 'NONE'
        pos = self.data[-1] if self.data else [0,0,0]*21
        print(label, '\n', pos)
    ...
    def update_img(self):
        ...
        if hands:
            ...
            tmp = []
            for lm in lmList:
                # tmp.extend([lm[0] - ox, h - (lm[1] - oy), lm[2] - oz])
                tmp.extend([lm[0], h - lm[1], lm[2]])
